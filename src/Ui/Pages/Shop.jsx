...
import shopdata from "/src/Ui/Components/shopdata.json";
import { Dropdown } from "flowbite-react";
import Cardcom from "/src/Ui/Components/CardCom";
...
export default function Shop() {
  ...
  const handleSizeChange = (e) => {
    ...
    if (isChecked) {
      ...
      setFilter({
        ...filter,
        size: filter.size.filter((size) => size !== sizeValue),
      });
    }
  };
    ...
    <>
      ...
      <div className="my-4 flex space-x-2 justify-center ">
        ...
        >
          ...
          <Dropdown.Item
            onClick={() => setFilter({ ...filter, brand: "newArrivals" })}
          >
            New Arrivals
          </Dropdown.Item>
          <Dropdown.Item
            onClick={() => setFilter({ ...filter, brand: "summerSale" })}
          >
            Summer Sale
          </Dropdown.Item>
          <Dropdown.Item
            onClick={() => setFilter({ ...filter, brand: "diwaliSale" })}
          >
            Diwali Sale
          </Dropdown.Item>
        </Dropdown>

        <Dropdown label="Category" className="bg-white border border-black">
          <Dropdown.Item
          ...
          <div className="px-4 py-2">
            ...
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                value="10ml"
                checked={filter.size.includes("10ml")}
                onChange={handleSizeChange}
              />
              <span>10 ml</span>
            </label>
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                value="50ml"
                checked={filter.size.includes("50ml")}
                onChange={handleSizeChange}
              />
              <span>50 ml</span>
            </label>
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                value="100ml"
                checked={filter.size.includes("100ml")}
                onChange={handleSizeChange}
              />
              <span>100 ml</span>
            </label>
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                value="150ml"
                checked={filter.size.includes("150ml")}
                onChange={handleSizeChange}
              />
              <span>150 ml</span>
            </label>
        ...
        <Dropdown label="Price" className="bg-white border border-black">
          <Dropdown.Item onClick={() => setFilter({ ...filter, price: "" })}>
            All
          </Dropdown.Item>
          <Dropdown.Item
            onClick={() =>
        ...
        <Dropdown
          ...
          className="bg-white border border-black"
        >
          <Dropdown.Item onClick={() => setFilter({ ...filter, gender: "" })}>
            All
          </Dropdown.Item>
          <Dropdown.Item
          ...
          <Dropdown.Item
            onClick={() => setFilter({ ...filter, gender: "kids" })}
          >
            Kids
          </Dropdown.Item>
        </Dropdown>
      ...
      <div className="container mx-auto px-4 py-3">
        <div className="relative pb-4 ">
          <img
            src="/src/assets/ulysse-pointcheval--j6LLsAehUo-unsplash.jpg"
            alt="Shop"
            className="h-[400px] w-full border rounded-3xl"
          />
          <div className="absolute inset-0 flex items-center justify-center">
            <i className=" mb-4 text-white text-9xl font-extrabold bg-transparent space-x-0">
              {" "}
              SHOP{" "}
            </i>
          </div>
